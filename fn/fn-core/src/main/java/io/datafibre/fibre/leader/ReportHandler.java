...
import io.datafibre.fibre.system.Backend.BackendStatus;
...
import io.datafibre.fibre.task.DropReplicaTask;
import io.datafibre.fibre.task.LeaderTask;
import io.datafibre.fibre.task.PublishVersionTask;
import io.datafibre.fibre.task.StorageMediaMigrationTask;
...
import io.datafibre.fibre.task.UpdateSchemaTask;
import io.datafibre.fibre.thrift.TBackend;
import io.datafibre.fibre.thrift.TColumn;
import io.datafibre.fibre.thrift.TDataCacheMetrics;
...
import io.datafibre.fibre.thrift.TResourceUsage;
...
public class ReportHandler extends Daemon implements MemoryTrackable {
    ...
    private static void tabletReport(long backendId, Map<Long, TTablet> backendTablets, long backendReportVersion) {
        ...
        GlobalStateMgr.getCurrentState().getTabletInvertedIndex().tabletReport(backendId, backendTablets, storageMediumMap,
                tabletSyncMap,
            ...
            while (offset < allTabletIds.size()) {
                ...
                try {
                    ...
                    for (int i = 0; i < tabletMetaList.size(); i++) {
                        ...
                        Partition partition =
                                globalStateMgr.getLocalMetastore().getPartitionIncludeRecycleBin(olapTable, partitionId);
                        if (partition == null) {
                            continue;
                        }
